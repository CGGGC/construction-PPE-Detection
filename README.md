...
학습된 각 모델의 성능을 정량적으로 평가하고, 그러한 결과가 도출된 원인을 모델 아키텍처 관점에서 분석했습니다.

### 📊 모델별 성능 요약

| 모델명    |   주요 평가지표               |       결과 및 원인 분석             |
| ---------- | ---------------------------- |------------------------------------|
| Faster R-CNN|  0.7766                     |  가장 안정적인 정확도                |
|            |  (안정적 성능)                | 객체가 있을 만한 영역을 먼저 제안하는 RPN(Region Proposal Network)을 거치는 2-Stage 구조 덕분에 미세한 객체나 겹친 객체에 대한 바운딩 박스 예측 정확도가 가장 높게 나타났습니다.|
| YOLOv8     | F1-Score                     | 실시간성 우수, 소수 클래스 취약 |
|            | Helmet 79.8%, Person 79.5%   | 이미지 전체의 맥락을 한 번에 파악하는 1-Stage 구조로 연산 속도가 압도적입니다. 하지만, 식별이 명확한 사람/헬멧은 잘 탐지하지만 데이터가 부족한 No-Helmet(14.8%) 등의 소수 클래스에서는 예측력이 떨어지는 한계를 보였습니다. |
| Custom Model| 상대적으로 낮은 성능          | 사전 학습된 Backbone 의 부재         |
|            |   mAP 및 F1-Score            | COCO 등 대규모 데이터로 사전 학습된 강력한 특징 추출기(Feature Extractor)가 부재한 상태에서 스크래치부터 학습했기 때문에 미탐지 및 오탐지가 다소 발생했습니다. |
...
* 실무 적용 관점 (Baseline Models):
실제 공사 현장 상황에 따라 적합한 모델이 다름을 확인했습니다. 즉각적인 경고 알림이 필요한 시스템에는 빠른 추론 속도의 YOLOv8이 적합하며, 사각지대나 멀리 있는 작업자의 정밀한 안전 관리가 필요한 경우 Faster R-CNN을 도입하는 것이 타당합니다.

* 연구 및 아키텍처 이해 관점 (Custom Model):
비록 커스텀 모델의 정량적 수치는 기성 모델에 미치지 못했으나, 본 프로젝트의 진정한 성과는 AI 모델의 내부 구조를 밑바닥부터 이해한 것에 있습니다. 프레임워크 API에 의존하지 않고 Target Encoding, Multi-anchor 매칭 로직 설계, Loss Function 최적화, NMS(Non-Maximum Suppression) 필터링 등 객체 탐지의 핵심 로직을 직접 구현해 보며 딥러닝 파이프라인 전반에 대한 깊은 통찰력을 얻을 수 있었습니다.
